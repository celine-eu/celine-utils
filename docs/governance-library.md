...
There was an `admin` extra until 2.3.0. It is gone with the command tree it served —
requesting it now installs successfully and warns `does not have an extra named
'admin'`, so check the name rather than trusting the install to complain.
